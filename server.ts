  ...
  for (let i = 0; i < totalAttempts; i++) {
    ...
    try {
      ...
      const model = genAI.getGenerativeModel({ 
        model: currentModel,
        generationConfig: options.config || options.generationConfig || { temperature: 0.7 }
      });
      ...
      if (typeof options.contents === 'string') {
        ...
        // Simplified mapping for the old SDK format
        parts = options.contents.flatMap((c: any) => c.parts || []);
      }
...
Always remain professional, helpful, and maintain a futuristic tech persona.`;
    ...
    if (!admin.apps.length) {
      ...
      if (admin.apps.length) {
        try {
          ...
          console.log(`[Firebase] Checking Admin SDK access for DB: ${dbId || '(default)'}...`);
          adminDb = currentAdminDb;
          
          // Background verification to avoid blocking startup
          (async () => {
            try {
              await Promise.race([
                currentAdminDb.collection("_system_").doc("health").get(),
  ...
  try {
    const geminiKey = await getGeminiApiKey();
    if (geminiKey) {
        const response = await callGeminiWithRetry(geminiKey, { contents: "Hi", model: "gemini-2.0-flash" });
        geminiStatus = response.text ? "✅ Gemini AI is working! (Gemini 2.0 Flash)" : "❌ Gemini returned empty response";
     } else {
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">
  ...
  app.get("/api/vamshavali/profile/:email", async (req, res) => {
    ...
    try {
      ...
      if (!profile) {
        if (email === "okbgmi611@gmail.com" || email === "ujirpur.barnia6@gmail.com") {
      ...
      } else if ((email === "okbgmi611@gmail.com" || email === "ujirpur.barnia6@gmail.com") && (!profile.members || profile.members.length === 0)) {
  ...
  app.post("/api/admin/approve-ai", async (req, res) => {
    ...
    try {
      ...
      if (adminDb) {
         ...
         isAdmin = adminSnap.exists && (adminSnap.data()?.email === "okbgmi611@gmail.com" || adminSnap.data()?.email === "ujirpur.barnia6@gmail.com");
      } else {
  ...
  try {
    ...
    if (action === "ADD") {
      ...
      if (member) {
        ...
        if (details.name && action === "UPDATE") member.name = details.name; // Support renaming members
...
----------------------------------
  ...
  app.post("/api/webhooks/telegram", async (req, res) => {
    ...
    if (text.toLowerCase() === '/status' || text.toLowerCase() === '/diagnostic' || text.toLowerCase() === '/info') {
        ...
        try {
           const response = await callGeminiWithRetry(geminiKey!, { contents: "Hi", model: "gemini-2.0-flash" });
           geminiStatus = response.text ? "✅ Gemini 2.0 ACTIVE" : "❌ Gemini Empty";
        } catch (e: any) {
      ...
      if (command.action === "CHAT") {
        ...
        if (isTextTask) {
          try {
            ...
            // Deduct credits for off-topic requests
            if (command.isOffTopic) {
               console.log("[Telegram] Charging for off-topic request...");
               const newCredits = (memory.credits || 0) - 5;
               await saveTelegramUserMemory(chatId, { credits: newCredits });
               await sendMsg(`⚡ *Action Charged:* 5 Credits deducted for this premium task. (New Balance: ${newCredits} CR)`);
            }
...
_Hint: try to be very specific, like 'Add Rahul as son of Sanjay' or 'Linked with VAM-XXXXX'_`;
  ...
  const AI_COSTS = {
    ...
    image_to_image: 20,
    image_to_video: 45,
    off_topic: 5 // Charge for non-website/service related requests
  };
  ...
  async function executeAIRouting(userId: string, task: string, type: string, inputImage: string | null, isApproved: boolean, userData: any) {
    ...
    const isAdmin = userEmail === "okbgmi611@gmail.com" || userEmail === "ujirpur.barnia6@gmail.com";
    ...
    const finalTask = finalType === "text" || !finalType 
      ? `[Barnali Guidelines] 
      PERSONA: You are Barnali, a professional salesperson for Barnia Digital Hub. Reply with politeness and authority.
      STRICT FOCUS: Only discuss our website, AI Router, and Genealogy (Family Tree) services.
      LEAD CAPTURE: If the user is interested in upgrades or premium support, ALWAYS ask for their Name, Email, and Phone number if you don't have it.
      MEMORY: You know the user is ${userData.name || 'valued customer'}.
      WEBSITE: https://barnia.in.
    ...
    if (type === "image") {
      ...
      try {
        ...
        if (sysGeminiKey) {
          ...
          if (!aiRes || !aiRes.text) {
             console.log("[AI-Router] Tier-1 Pro failed, trying Flash Lite...");
