    ...
    try {
      ...
      try {
        ...
        if (geminiKey) {
           ...
           geminiStatus = response.text ? "✅ Gemini AI is working! (3-flash-preview)" : "❌ Gemini returned empty response";
        } else {
    ...
    try {
      ...
      if (text.toLowerCase() === '/status' || text.toLowerCase() === '/diagnostic' || text.toLowerCase() === '/info') {
        ...
        try {
          ...
          const response = await aiTest.models.generateContent({ model: "gemini-3-flash-preview", contents: "Hi" });
...
_Use '/link <id>' if features are missing._`);
      ...
      if (command.action === "CHAT") {
        ...
        try {
          ...
          // CRITICAL FALLBACK: If Router Hub fails but it is a text request, try the direct Gemini Fallback
          // to ensure Barnali is never "down" for the user.
          if (command.taskType === "text" || !command.taskType) {
            try {
              console.log("[Telegram] Router failed, attempting Native Gemini fallback...");
              let fallbackPrompt = `You are Barnali, the family archive assistant. Help with: ${command.userQuestion || text}`;
              const fallbackRes = await callGeminiWithRetry(geminiKey, {
                contents: [{ role: 'user', parts: [{ text: fallbackPrompt }] }]
              });
              const textOutput = fallbackRes.text || "I am here to help, but my advanced routing logic is resetting. How can I assist you with your family tree?";
              await sendMsg(`💌 *Barnali (Caretaker Mode):* ${textOutput}\n\n_(via Direct Fallback)_`);
            } catch (gemErr) {
               await sendMsg(`⚠️ *System Overload:* Even my backup systems are busy. Please try again in a few minutes!`);
            }
          } else {
  ...
  async function executeAIRouting(userId: string, task: string, type: string, inputImage: string | null, isApproved: boolean, userData: any) {
    ...
    if (type === "image") {
      ...
      if (!response && process.env.DASHSCOPE_API_KEY) {
        response = await getImg2ImgResponse(task, ""); // Passing empty image triggers text-to-image in Alibaba helper
      }
