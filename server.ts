    ...
    try {
      ...
      try {
        ...
        if (geminiKey) {
           ...
           geminiStatus = response.response.text() ? "✅ Gemini AI is working! (1.5-flash-latest)" : "❌ Gemini returned empty response";
        } else {
    ...
    try {
      ...
      if (text.toLowerCase() === '/status' || text.toLowerCase() === '/diagnostic' || text.toLowerCase() === '/info') {
        ...
        try {
          ...
          const model = aiTest.getGenerativeModel({ model: "gemini-1.5-flash-latest" });
          const response = await model.generateContent("Hi");
...
_Use '/link <id>' if features are missing._`);
      ...
      const prompt = `Barnali AI Router. Archive Keeper & Smart Assistant.
      ...
      3. taskType: "image" if they want a picture, "video" for video, else "text".
      
      Examples:
      - 'Who are you?' -> { "action": "CHAT", "taskType": "text", "userQuestion": "Introduce yourself as Barnali, the family archive keeper" }
      - 'Make a photo of a temple' -> { "action": "CHAT", "taskType": "image", "userQuestion": "Beautiful ancient Indian temple" }
