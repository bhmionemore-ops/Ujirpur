    ...
    if (!admin.apps.length) {
        try {
          ...
          if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
            ...
            console.log("[Firebase] No credentials found, using project-only init.");
            admin.initializeApp({ projectId: firebaseConfig.projectId });
          }
          console.log("[Firebase] Admin SDK initialized.");
        } catch (initError: any) {
          console.warn(`[Firebase] Admin SDK initialization unsuccessful: ${initError.message}. We will rely on Client SDK.`);
        }
      }
      ...
      if (admin.apps.length) {
        try {
          ...
          // Verify Admin SDK with a test read to catch "7 PERMISSION_DENIED" early
          console.log("[Firebase] Verifying Admin SDK permissions...");
          try {
            await currentAdminDb.collection("_health_check").limit(1).get();
            adminDb = currentAdminDb;
            console.log("[Firebase] Admin SDK verified and assigned.");
          } catch (verifyError: any) {
            console.warn(`[Firebase] Admin SDK verification FAILED: ${verifyError.message}`);
            if (verifyError.message.includes("PERMISSION_DENIED") || verifyError.code === 7) {
              console.warn("[Firebase] Admin SDK has INSUFFICIENT PERMISSIONS. Disabling it to allow Client SDK fallback system-wide.");
              adminDb = null;
            } else {
              adminDb = currentAdminDb; // Keep it if it's some other non-fatal error
            }
          }
        } catch (dbError: any) {
